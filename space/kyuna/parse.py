        ...
        if i == 1:
            if len(table.columns) > 1:
                ...
                for column_name, column in df.items():
                    ...
                    if pd.isna(column[label.index("진단 Event 명칭")]):
                        continue
                    else :
                        event = [{
            ...
            if len(table.columns) > 1:
                df  = table.iloc[: , 1:]
                for column_name, column in df.items():
                    if pd.isna(column[label.index("Event Path 명칭")]):
                        continue
                    else :
                        path = [{
                                "SYSCON": column[label.index("System Constant 조건")] if pd.notna(column[label.index("System Constant 조건")]) else "",  # [3]System Constant 조건
            ...
            if len(table.columns) > 1:
                ...
                for column_name, column in df.items():    # column_name : Series, column : Series
                    if pd.isna(column[label.index("함수 식별자 명칭")]):
                        continue
                    else :
                        fid = [{
            ...
            if len(table.columns) > 1:
                df  = table.iloc[: , 1:]
                for column_name, column in df.items():
                    if pd.isna(column[label.index("DTR test 명칭")]):
                        continue
                    else :
                        dtr = [{
                                "SYSCON": column[label.index("System Constant 조건")] if pd.notna(column[label.index("System Constant 조건")]) else "",  # [3]System Constant 조건
            ...
            if len(table.columns) > 1:
                df  = table.iloc[: , 1:]
                for column_name, column in df.items():
                    if pd.isna(column[label.index("신호 명칭")]) :
                        continue
                    else :
                        sig = [{
