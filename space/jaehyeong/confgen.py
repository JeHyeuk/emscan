    ...
    for FID in Fid_list:
        for element in FID:
            if isinstance(element, dict):  # 리스트 Read 형식 변환
                ...
                # Unused일 때, %afimd_conf 는 표출되고, %ftrme_conf 는 표출되지 않음 ▶ X와 Unused는 SW에 사용되지 않는 같은 의미로 보며, 하기 로직 표출 안하기로 결정@@@@
                if element.get("FID_GROUP") not in ["X", "Unused", None, ""]:
                    ...
                    if "IUMPR_EVENT" in element and element["IUMPR_EVENT"]:  # IUMPR_EVENT 존재하고 비어 있지 않으면
                        if any(event.strip() for event in element["IUMPR_EVENT"]):  # 빈 값만 있는 경우 출력 생략
                            if isinstance(element["IUMPR_EVENT"], list):
                                iumpr_event_list = element["IUMPR_EVENT"]
                                iumpr_syscon_list = element["IUMPR_EVENT_SYSCON"]
                            else:
                                iumpr_event_list = [element["IUMPR_EVENT"]]
                                iumpr_syscon_list = [element["IUMPR_EVENT_SYSCON"]]

                            for w, event in enumerate(iumpr_event_list):
                ...
                if "INHIBITED_EVENT" in element and element["INHIBITED_EVENT"]:  # INHIBITED_SIGS가 존재하고 비어 있지 않으면
                    if any(event.strip() for event in element["INHIBITED_EVENT"]):  # 빈 값만 있는 경우 출력 생략
                        if isinstance(element["INHIBITED_EVENT"], list):
                            event_list = element["INHIBITED_EVENT"]
                        ...
                        for i, event in enumerate(event_list):
                            if event.strip():  # 값이 있는 경우만 처리
                                ...
                                else:  # event_mask 값이 없을 때
                                    f.write(f'										<VF>{event}</VF>\n')

                                f.write(f'									</CONF-ITEM>\n')

                # INHIBITED_SUM_EVENT 값이 있을 때만 추가
                if "INHIBITED_SUM_EVENT" in element and element[
                    "INHIBITED_SUM_EVENT"]:  # INHIBITED_SUM_EVENTS가 존재하고 비어 있지 않으면
                    if any(sum.strip() for sum in element["INHIBITED_SUM_EVENT"]):  # 빈 값만 있는 경우 출력 생략
                        if isinstance(element["INHIBITED_SUM_EVENT"], list):
                            sum_event_list = element["INHIBITED_SUM_EVENT"]
                        ...
                        for k, sum_event in enumerate(sum_event_list):
                            if sum_event.strip():  # 값이 있는 경우만 처리
                                sum_event_mask = mask_list[k] if k < len(mask_list) else None  # 마스크 정보가 없으면 None으로 처리
